  ...
  type = map(object(
    {
      ...
      profiles                               = optional(list(strings))
...
module "bios_policy_nvmeof" {
  depends_on = [
    local.org_moids
  ]
  source       = "terraform-cisco-modules/imm/intersight//modules/policies_bios"
  for_each     = var.assign_best_practice_bios == true ? local.best_practice_bios_policies : {}
  description  = each.value.nvmeof_description != "" ? each.value.nvmeof_description : "${each.value.organization} BIOS Policy for NVMeoF."
  organization = local.org_moids[each.value.organization].moid
  profiles     = each.value.bios_nvmeof_profiles
  tags         = each.value.nvmeof_tags != [] ? each.value.nvmeof_tags : local.tags
  #+++++++++++++++++++++++++++++++
...
module "bios_policy_vmware_node" {
  depends_on = [
    local.org_moids
  ]
  source       = "terraform-cisco-modules/imm/intersight//modules/policies_bios"
  for_each     = var.assign_best_practice_bios == true ? local.best_practice_bios_policies : {}
  description  = each.value.vmware_node_description != "" ? each.value.vmware_node_description : "${each.value.organization} BIOS Policy for VMware Node."
  organization = local.org_moids[each.value.organization].moid
  profiles     = each.value.profiles
  tags         = each.value.vmware_node_tags != [] ? each.value.vmware_node_tags : local.tags
  #+++++++++++++++++++++++++++++++
...
module "bios_policy_vmware_rack" {
  depends_on = [
    local.org_moids
  ]
  source       = "terraform-cisco-modules/imm/intersight//modules/policies_bios"
  for_each     = var.assign_best_practice_bios == true ? local.best_practice_bios_policies : {}
  description  = each.value.vmware_rack_description != "" ? each.value.vmware_rack_description : "${each.value.organization} BIOS Policy for VMware Rackmount."
  organization = local.org_moids[each.value.organization].moid
  profiles     = each.value.profiles
  tags         = each.value.vmware_rack_tags != [] ? each.value.vmware_rack_tags : local.tags
  #+++++++++++++++++++++++++++++++
  # Boot Options Section
  #+++++++++++++++++++++++++++++++
  vmd_enable = "enabled"          # VMD Enablement
...
module "custom_bios_policy" {
  depends_on = [
    local.org_moids
  ]
  source      = "terraform-cisco-modules/imm/intersight//modules/policies_bios"
  for_each    = var.create_custom_bios_policy == true ? local.custom_bios_policy : {}
