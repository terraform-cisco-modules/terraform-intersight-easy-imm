...
variable "ethernet_qos_policies" {
  ...
  * burst - Defualt is 1024.  The burst traffic, in bytes, allowed on the vNIC.  Value can be between 1024-1000000.
  ...
  * enable_trust_host_cos - Defualt is false.  Enables usage of the Class of Service provided by the operating system.
  * mtu - Defualt is 1500.  The Maximum Transmission Unit (MTU) or packet size that the virtual interface accepts.  Value can be between 1500-9000.
  * organization - Name of the Intersight Organization to assign this Policy to.
    - https://intersight.com/an/settings/organizations/
  * priority - Defualt is 'Best Effort'.  The priortity matching the System QoS specified in the fabric profile.  Options are:
    - Platinum
    - Gold
    - Silver
    - Bronze
    - Best Effort
    - FC
  * rate_limit - Defualt is 0.  The value in Mbps (0-10G/40G/100G depending on Adapter Model) to use for limiting the data rate on the virtual interface. Setting this to zero will turn rate limiting off.  Range is between 0-100000.
