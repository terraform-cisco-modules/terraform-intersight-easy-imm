...
module "ucs_chassis_profiles" {
  ...
  source = "../../../terraform-intersight-imm/modules/chassis_profile"
  ...
  assign_chassis = each.value.profile.assign_chassis == true ? [
    {
      moid        = data.intersight_equipment_chassis.chassis[each.key].results[0].moid
      object_type = data.intersight_equipment_chassis.chassis[each.key].results[0].source_object_type
    }
  ] : []
