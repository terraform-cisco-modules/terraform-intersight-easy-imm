  ...
  type = map(object(
    {
      ...
      fan_control_mode = optional(number)
...
module "policy_thermal" {
  ...
  local.ucs_chassis_profile)) : module.ucs_chassis_profile[s].moid if local.ucs_chassis_profile[s].profile.policy_thermal == each.key]
}
