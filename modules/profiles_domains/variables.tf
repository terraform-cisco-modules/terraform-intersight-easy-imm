...
variable "ucs_domain_profile" {
  default = {
    default = { # The UCS Domain Profile Name will be {each.key}.  In this case it would be default if left like this.
      assign_fabric_interconnects        = false
  ...
  1. assign_fabric_interconnects - Flag to define if the physical FI's should be assigned to the policy or not at this time.
  ...
  7. For the remainder of the option documentation refer to these sources:
  ...
  type = map(object(
    {
      assign_fabric_interconnects        = optional(bool)
...
variable "ucs_domain_profile" {
  ...
  2. config_fibre_channel = Flag to define if fibre-channel should be configured for the UCS Domain.
  3. organization - Name of the Intersight Organization to assign this pool to:
  4. snmp_configure - Flag to define if SNMP should be assigned to the Domain or not.
  5. snmp_type - There are three SNMP modules: snmp_community, snmp_1_user, and snmp_2_users.  Use this field to define which policy to assign to the domain when snmp_configure is set to true.
  6. syslog_configure - Flag to define if Syslog should be assigned to the Domain or not.
