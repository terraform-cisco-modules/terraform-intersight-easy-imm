...
module "system_qos_policies_1" {
  ...
  profiles = flatten([
    ...
    if local.ucs_domain_profiles[s].profile.system_qos_policies == each.key
  ])
  classes = []
}

module "system_qos_policies_2" {
  depends_on = [
    local.org_moids,
    module.ucs_domain_profiles_a,
    module.ucs_domain_profiles_b
  ...
  #   for s in sort(keys(local.ucs_domain_profiles)) :
  #   [module.ucs_domain_profiles_a[s].moid, module.ucs_domain_profiles_b[s].moid]
  #   if local.ucs_domain_profiles[s].system_qos_policies == each.key
  # ])
  classes = [
    {
