...
module "snmp_policies" {
  ...
  profiles = [
    ...
    if local.ucs_server_profiles[s].profile.snmp_policies == each.key
  ]
}

module "snmp_policies_1_user" {
  depends_on = [
    local.org_moids,
    module.ucs_server_profiles
  ]
  ...
  source                  = "../../../terraform-intersight-imm/modules/snmp_policies_1_user"
  description             = each.value.description != "" ? each.value.description : "${each.key} SNMP Policy."
  enabled                 = each.value.enabled
  name                    = each.key
  org_moid                = local.org_moids[each.value.organization].moid
...
module "snmp_policies_2_users" {
  ...
  source                  = "../../../terraform-intersight-imm/modules/snmp_policies_2_users"
  description             = each.value.description != "" ? each.value.description : "${each.key} SNMP Policy."
  enabled                 = each.value.enabled
  name                    = each.key
  org_moid                = local.org_moids[each.value.organization].moid
