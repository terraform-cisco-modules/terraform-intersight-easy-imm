...
locals {
  ...
  tags = var.tags
  bios_nvmeof_orgs = {
    for v in local.organizations : v =>
    {
      for orgs, org in local.ucs_server_profiles : orgs => org
      if org.organization == v  && org.bios_policy == "nvmeof" ? true : false
    }
  }
  bios_nvmeof_profiles = {
    for policy, assign in local.ucs_server_profiles : policy => assign
    if assign.bios_policy == "nvmeof" ? true : false
  }
  bios_vmware_node_orgs = {
    for v in local.organizations : v =>
    {
      for orgs, org in local.ucs_server_profiles : orgs => org
      if org.organization == v && org.bios_policy == "vmware_node" ? true : false
    }
  }
  # bios_vmware_node_profiles = {
  #   for policy, assign in local.ucs_server_profiles : policy => assign
  #   if assign.bios_policy == "vmware_node" ? true : false
  # }
  ...
  ucs_server_profiles = {
    for k, v in var.ucs_server_profiles : k => {
      ...
      bios_policy         = (v.bios_policy != null ? v.bios_policy : "vmware_node")
