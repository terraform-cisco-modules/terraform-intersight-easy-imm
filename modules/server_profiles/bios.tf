...
module "bios_policy_nvmeof" {
  ...
  for_each = {
    for assign, bios_policy in local.organization_variables : assign => bios_policy
    if bios_policy.bios_nvmeof == true
  }
  description  = each.value.bios_nvmeof_descr != "" ? each.value.nvmeof_description : "${each.key} BIOS Policy for NVMeoF."
  org_moid     = local.org_moids[each.key].moid
  profiles     = [ for s in sort(keys(local.bios_nvmeof_profiles)) : module.ucs_server_profile[s].moid ]
...
module "bios_vmware_node" {
  ...
  for_each = {
    for assign, bios_policy in local.organization_variables : assign => bios_policy
    if bios_policy.bios_vmware_node == true
  }
  description  = each.value.bios_vmware_node_descr != "" ? each.value.bios_vmware_node_descr : "${each.key} BIOS Policy for VMware Node."
  org_moid     = local.org_moids[each.key].moid
  profiles     = [ for s in sort(keys(local.bios_vmware_node_profiles)) : module.ucs_server_profile[s].moid ]
  tags         = each.value.tags != [] ? each.value.tags : local.tags
  #+++++++++++++++++++++++++++++++
...
module "bios_vmware_rack" {
  ...
  for_each = {
    for assign, bios_policy in local.organization_variables : assign => bios_policy
    if bios_policy.bios_vmware_rack == true
  }
  description  = each.value.bios_vmware_rack_descr != "" ? each.value.bios_vmware_rack_descr : "${each.key} BIOS Policy for VMware Rackmount."
  org_moid     = local.org_moids[each.key].moid
  profiles     = [ for s in sort(keys(local.bios_vmware_rack_profiles)) : module.ucs_server_profile[s].moid ]
  tags         = each.value.tags != [] ? each.value.tags : local.tags
  #+++++++++++++++++++++++++++++++
  # Boot Options Section
  #+++++++++++++++++++++++++++++++
...
module "bios_custom" {
  ...
  for_each = {
    for assign, bios_policy in local.organization_variables : assign => bios_policy
    if bios_policy.bios_custom == true
  }
  description = each.value.description
  name        = each.bios_custom.name
  org_moid    = local.org_moids[each.key].moid
  profiles     = [ for s in sort(keys(local.bios_vmware_rack_profiles)) : module.ucs_server_profile[s].moid ]
  tags        = each.value.tags != [] ? each.value.tags : local.tags
  #+++++++++++++++++++++++++++++++
