...
variable "policy_serial_over_lan" {
  default = {
    default = {
      baud_rate    = 115200
      com_port     = "com0"
...
module "policy_serial_over_lan" {
  ...
  profiles    = [for s in sort(keys(
    local.ucs_server_profiles)) : module.ucs_server_profile[s].moid if local.ucs_server_profiles[s].policy_serial_over_lan == each.key]
